...
-- custom exclusion
function WayHandlers.exclude_way(profile, way, result, data)
  local type = way:get_value_by_key('highway')
end
...
function WayHandlers.classes(profile,way,result,data)
    ...
    if allowed_classes["steps"] and data.highway == "steps" then
      io.stderr:write("DEBUG: Steps class applied to way ID: ", way:id(), "\n")
      result.forward_classes["steps"] = true
      result.backward_classes["steps"] = true
    end
    if allowed_classes["indoor"] and data.highway == "indoor" then
      result.forward_classes["indoor"] = true
      result.backward_classes["indoor"] = true
    end
